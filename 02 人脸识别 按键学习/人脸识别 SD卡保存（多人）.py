...
while jj != zz:
    jj = aaa.find('$',ii,zz)
    if(jj != zz):
        record_ftrs.append(bytearray(aaa[ii:jj]))
    else:
        record_ftrs.append(bytearray(aaa[ii:jj-1]))
    jj+=1
    ii=jj
print(record_ftrs)
    ...
    if code: # 如果检测到人脸
        for i in code: # 迭代坐标框
            ...
            if max_score > 85: # 如果最大分数大于85， 可以被认定为同一个人
                a = img.draw_string(i.x(),i.y(), ("%s :%2.1f" % (names[index], max_score)), color=(255,255,255),scale=2) # 显示人名 与 分数
            else:
            ...
            if key_pressed == 1: #如果检测到按键
                ...
                ff1 = open('/sd/features.txt','w',encoding='utf-8')  #,encoding='utf-8'
