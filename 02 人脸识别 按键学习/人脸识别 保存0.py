...
anchor = (1.889, 2.5245, 2.9465, 3.94056, 3.99987, 5.3658, 5.155437, 6.92275, 6.718375, 9.01025) #人脸定位 用于人脸检测的 锚
dst_point = [(44,59),(84,59),(64,82),(47,105),(81,105)] #标准面关键点位置 标准正脸的5关键点坐标 分别为 左眼 右眼 鼻子 左嘴角 右嘴角
...
'''
fp = open('/sd/features.txt','r') #,encoding='utf-8'
record_ftrs.append(fp.read())
fp.close()
'''
    ...
    if code: # 如果检测到人脸
        for i in code: # 迭代坐标框
            ...
            if max_score > 85: # 如果最大分数大于85， 可以被认定为同一个人
                a = img.draw_string(10,10, ("%s :%2.1f" % (record_ftrs[index], max_score)), color=(255,255,255),scale=2) # 显示人名 与 分数
                print(record_ftrs[index])
                print(type(record_ftrs[0]))
# a = img.draw_string("%s :%2.1f" % (record_ftrs[index],color=(0,0,0),scale=2)
            else:
            ...
            if key_pressed == 1: #如果检测到按键
                ...
                ff1 = open('/sd/features.txt','w')  #,encoding='utf-8'
